...
{
	$rowid = $_POST['rowid'];
	$QUERY = "SELECT * FROM dbc_order_request WHERE request_id='$rowid'";
...
.form-wrapper {width:400px;height:390px;overflow-y:auto; position:relative}
...
<div class="form-wrapper">
	<table style="width: 100%" class="table">
		<!--tr>
			<th>Request Type:</th>
			<td>
				<select id="request_type" class="form-control" disabled>
		...
		<tr>
			<th>Date:</th>
			<td><input id="trans_date" type="date" class="form-control" value="<?php echo $trans_date; ?>" disabled></td>
		</tr>
		<tr>
			<th>Branch:</th>
		...
		<tr>
			<th>Recipient:</th>
			<td>
				<!--select id="reci_pient" class="form-control">
					<?php echo $function->GetRecipient($reci_pient,$form_type,$db); ?>
				</select-->
				<input id="reci_pient" type="text" class="form-control" value="DAVAO BAKING CENTER" disabled>
			</td>
		</tr>
		<tr>
			<th>Created By:</th>
	...
	<?php if($request_type == 'new') { ?>
		<button class="btn btn-success" onclick="generateRequest('new')">
		Generate Request</button>
	<?php } if($request_type == 'edit') { ?>
		<button class="btn btn-info" onclick="generateRequest('edit')">Update Request</button>
		<button class="btn btn-warning" onclick="voidRequest()">Void Request</button>
	<?php } ?>
