...
if(isset($_POST['search']) AND isset($_POST['branch']) AND $_POST['branch'] != '')
{
	$search = $_POST['search'];	
	$branch = $_POST['branch'];	
	$q = "WHERE (recipient LIKE '%$search%' OR control_no LIKE '%$search%') AND branch='$branch' AND status='Closed'";
}
...
{
	$search = $_POST['search'];	
	$q = "WHERE (recipient LIKE '%$search%' OR control_no LIKE '%$search%') AND cluster='$cluster' AND status='Closed'";
}
else 
{
	if($userlevel >= 50)
	{
		$q = "WHERE cluster='$cluster' AND status='Closed' ORDER BY priority DESC $limit";
	} else {
		$q = "WHERE branch='$branch' AND status='Closed' ORDER BY priority DESC $limit";
	}
}
?>
<style>
...
<script>
function viewOrderRecord(controlno)
{
	var module = '<?php echo MODULE_NAME; ?>';
	$.post("./Modules/" + module + "/includes/my_orders_data.php", { control_no: controlno },
	function(data) {		
		$('#smnavdata').html(data);
	});	
}
