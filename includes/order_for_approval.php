...
<div class="smnav-header">
	...
	<div class="search-shell" style="margin-left:0px;width:200px;">
		<input id="branch" type="text" style="text-align:center" class="form-control form-control-sm" value="<?php echo $_SESSION['dbc_branch_branch']; ?>" disabled>
	</div>
	<?php } ?>
...
function selectBranch(value)
{
	...
	else
	{
		...
		$.post("./Modules/" + module + "/includes/order_approval_data.php", { limit: limit, branch: value },
		function(data) {
			$('#smnavdata').html(data);
			rms_reloaderOff();
		});
...
$(function()
{
	...
	$('#search').keyup(function()
	{
		...
		$.post("./Modules/" + module + "/includes/order_approval_data.php", { limit: limit, search: search, branch: branch },
		function(data) {
			$('#smnavdata').html(data);
		});
...
function load_data()
{
	...
	var limit = $('#limit').val();
	rms_reloaderOn("Loading data...");
	$.post("./Modules/" + module + "/includes/order_approval_data.php", { limit: limit },
	function(data) {
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});
