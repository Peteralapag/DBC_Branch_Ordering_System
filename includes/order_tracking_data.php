...
if(isset($_POST['search']) AND isset($_POST['branch']) AND $_POST['branch'] != '')
{
	$search = $_POST['search'];	
	$branch = $_POST['branch'];	
	$q = "WHERE (cluster='$cluster' AND branch='$branch' AND (status='In-Transit' OR  status='Submitted') AND recipient LIKE '%$search%' OR control_no LIKE '%$search%')'";
}
else if(!(isset($_POST['search'])) AND isset($_POST['branch']))
{
	$branch = $_POST['branch'];	
	$q = "WHERE cluster='$cluster' AND  branch='$branch' AND (status='In-Transit' OR  status='Submitted') ORDER BY priority DESC $limit";
}
else if(isset($_POST['search']) AND $_POST['branch'] == '')
{
	$search = $_POST['search'];	
	$q = "WHERE (recipient LIKE '%$search%' OR control_no LIKE '%$search%') AND cluster='$cluster' AND (status='In-Transit' OR  status='Submitted')";
}
else 
{
	if($userlevel >= 50)
	{
		$q = "WHERE branch='$branch' AND (status='In-Transit' OR  status='Submitted') ORDER BY priority,trans_date DESC $limit";
	} else {
		$q = "WHERE branch='$branch' AND (status='In-Transit' OR status='Submitted') ORDER BY priority,trans_date DESC $limit";
	}
}
...
<table style="width: 100%" class="table table-bordered table-striped table-hover">
	...
	<tbody>
<?PHP
	$sqlQuery = "SELECT * FROM dbc_order_request $q";
...
?>			
		<tr ondblclick="orderTracking('<?php echo $control_no; ?>')">
			...
			<td>
				<div class="change-btn btn-info" onclick="orderTracking('<?php echo $control_no; ?>')">Track</div>
			</td>
		</tr>
<?PHP 	} } else { ?>
		<tr>
			<td colspan="10" style="text-align:center"><i class="fa fa-bell"></i>&nbsp;&nbsp;No Order to track</td>
		</tr>
<?PHP }
	mysqli_close($db); 
?>			
