...
{
	$search = $_POST['search'];	
	$q = "WHERE branch='$branch' AND status='Open' AND recipient LIKE '%$search%' OR control_no LIKE '%$search%'";
} else {
	$q = "WHERE branch='$branch' AND status='Open' AND trans_date >= '$dateNow' ORDER BY priority DESC $limit";
}
...
?>			
		<tr ondblclick="editRequest('edit','<?php echo $rowid; ?>')">
			...
			<td>
				<div class="change-btn btn-info" onclick="editRequest('edit','<?php echo $rowid; ?>')">Edit</div>
			</td>
		</tr>
<?PHP 	} } else { ?>
		<tr>
...
function editRequest(params,rowid)
{
	...
	$.post("./Modules/" + module + "/apps/request_form.php", { params: params, rowid: rowid },
	function(data) {		
...
{
	var module = '<?php echo MODULE_NAME; ?>';
	$.post("./Modules/" + module + "/includes/request_data.php", { rowid: rowid  },
	function(data) {		
		$('#smnavdata').html(data);
	});	
}
